 ...
 * team
 * 
 * @author Jamie Lee Date: April 23, 2020
 */
...
//		drawButtons();
	...
	public void mouseClicked()
	{
		...
		List<Marker> allMarkers = new ArrayList<>(cityMarkers);

		allMarkers.addAll(quakeMarkers);

		lastClicked = (CommonMarker) getMarkerMouseClicked(allMarkers);

		if(lastClicked instanceof EarthquakeMarker) {

		hideInNonThreatCircleOfEarthquake((EarthquakeMarker)lastClicked, allMarkers);

		}

		if(lastClicked instanceof CityMarker) {

		hideNotInThreatCircleOfCityquake((CityMarker)lastClicked);

		}

		}

	private Marker getMarkerMouseClicked(List<Marker> markers) {
	...
	private void hideInNonThreatCircleOfEarthquake(EarthquakeMarker earthquake, List<Marker> allMarkers)

	{

		double threat = earthquake.threatCircle();

		for (Marker marker : allMarkers) {

			if (earthquake.getDistanceTo(marker.getLocation()) > threat) {
	...
	private void hideNotInThreatCircleOfCityquake(CityMarker cityMarker) {

		for (Marker quakeMarker : quakeMarkers) {

			double threat = ((EarthquakeMarker) quakeMarker).threatCircle();

			if (((EarthquakeMarker) quakeMarker).getDistanceTo(cityMarker.getLocation()) > threat) {
